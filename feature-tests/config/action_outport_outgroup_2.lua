--[[
  Feature test for simultaneously forwarding a packet to a second group and an output
]]
...
Feature.flowEntries = function(openflowVersion, flowData, outPort)
    table.insert(flowData.groups, string.format("group_id=2, type=all, bucket=mod_nw_dst=%s,output:%s", conf.new_DST_IP4, outPort))
    table.insert(flowData.groups, string.format("group_id=1, type=all, bucket=mod_nw_src=%s,group:2,output:%s", conf.new_SRC_IP4, outPort))
