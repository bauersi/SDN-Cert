...
Feature.flowEntries = function(openflowVersion, flowData, outPort)
    table.insert(flowData.flows, string.format("table=0, priority=1, dl_type=%s, actions=set_field:%s->dl_src, output:%s", Feature.enum.ETH_TYPE.wol, conf.new_SRC_MAC, outPort))
    table.insert(flowData.flows, string.format("table=0, priority=3, ip, actions=set_field:%s->nw_src, output:%s", conf.new_SRC_IP4, outPort))
    table.insert(flowData.flows, string.format("table=0, priority=2, ip, udp, actions=set_field:%s->udp_src, output:%s", conf.new_SRC_PORT, outPort))
  end
...
Feature.evalCounters = function(ctrs, batch, threshold)
    return (Feature.eval(ctrs[1],batch/2,threshold) and Feature.eval(ctrs[2],batch/2,threshold) and Feature.eval(ctrs[3], 0, threshold))
  end
