...
Feature.flowEntries = function(openflowVersion, flowData, outPort)
    table.insert(flowData.flows, string.format("table=0, ipv6, ipv6_src=%s, ipv6_dst=%s, actions=DROP", Feature.pkt.SRC_IP6, Feature.pkt.DST_IP6))
    table.insert(flowData.flows, string.format("table=0, ipv6, ipv6_src=%s, ipv6_dst=%s, actions=output:%s", conf.new_SRC_IP6, conf.new_DST_IP6, outPort))
  end
