...
Feature.flowEntries = function(openflowVersion, flowData, outPort)
    table.insert(flowData.flows, string.format("table=0, ip, ip_proto=%s, actions=DROP", Feature.pkt.PROTO))
    table.insert(flowData.flows, string.format("table=0, ip, ip_proto=%s, actions=output:%s", conf.new_PROTO, outPort))
  end
