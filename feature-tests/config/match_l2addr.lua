...
Feature.flowEntries = function(openflowVersion, flowData, outPort)
    ...
    table.insert(flowData.flows, string.format("%sdl_src=%s, dl_dst=%s, actions=DROP", tableId, Feature.pkt.SRC_MAC, Feature.pkt.DST_MAC))
    table.insert(flowData.flows, string.format("%sdl_src=%s, dl_dst=%s, actions=output:%s", tableId, conf.new_SRC_MAC, conf.new_DST_MAC, outPort))
  end
