...
function master(featureName, ...)
  ...
  for i=1,#ports do
    ...
    txQueues[i] = dev:getTxQueue(0)
    rxQueues[i] = dev:getRxQueue(0)
  end
  device.waitForLinks()
  for i=1,#txQueues do
    local rate = 100 - (settings.pktSize + 4) * 8 / 1000
    txQueues[i]:setRate(rate)
  end

  mg.startTask("featureTxSlave", featureName, txQueues, ports)
  mg.startTask("featureRxSlave", featureName, rxQueues, ports)
  mg.waitForTasks()
end
...
function evaluate(featureName, rxPkts, rxCtrs, ports)
  ...
  for p=1,#rxPkts do
    local pkt = rxPkts[p]
    if pkt.eth_type == 0x88cc then
      print("Ignore 'Link Layer Discovery Protocol (LLDP)' packet")
    else
      local index = 0
...
function featureTxSlave(featureName, txQueues, ports)
  importFeature(featureName)

  mg.sleepMillis(500)     -- important! ensures that rx starts before tx

  local txCtrs = {}

  -- check how many tx iterations are performed
  local txSteps = settings.txIterations
  if (not txSteps or txSteps <= 0) then txSteps = #txQueues end
  ...
  for n=1,txSteps do
    ...
    txCtrs[txPkt.TX_DEV] = stats:newDevTxCounter(txQueues[txPkt.TX_DEV], "plain")
...
function featureRxSlave(featureName, rxQueues, ports)
  importFeature(featureName)
  local mempool = memory.createMemPool(function(buf)
  ...
  if (learnFrames > 0) then 
    ...
    mg.sleepMillis((settings.learnTime or 500) + 500)
    for i=settings.firstRxDev,#rxQueues do
  ...
  for i=firstRxDev,#rxQueues do rxCtrs[i] = stats:newDevRxCounter(rxQueues[i], "plain") end
  local timeout = mg.getTime() + settings.timeout
  local rxPkts = {}
  local rxPattern = {}
