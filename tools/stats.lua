...
local function getPlainUpdate(direction)
	return function(stats, file, time, totalPkt, totalBytes, mpps, mbit, wireMbit)
...
local function getCsvInit(direction)
	return function(stats, file)
		if not headersShown[file] then
			...
			file:write("time, packets, bytes, mpps, mbit, wirembit\n")
		end
	end
end
...
local function getCsvUpdate(direction)
	return function(stats, file, time, totalPkt, totalBytes, mpps, mbit, wireMbit)
		file:write(("%s, %s, %s, %s, %s, %s\n"):format(time, totalPkt, totalBytes, mpps, mbit, wireMbit))
		file:flush()
	end
end
...
local function updateCounter(self, time, pkts, bytes, dontPrint)
	...
	if not dontPrint then
		self:print("Update", active, self.total, self.totalBytes, mpps, mbit, wireRate)
	end
