...
function master(testId, txPort, rxPort, duration, rate, size, flowpattern, flowpattern_args, udp_dst_port)
	...
	-- create traffic
	mg.startTask("loadSlave", testId, txDev:getTxQueue(0), size, duration+2, 1, flowpattern, flowpattern_args) -- +1 wegen Anzeige
...
function loadSlave(id, queue, size, duration, wait, flowpattern, flowpattern_args)
...
function timerSlave(id, txQueue, rxQueue, size, duration, wait, flowpattern, flowpattern_args, udp_dst_port)
	...
	local timestamper
	if udp_dst_port ~= nil then
		timestamper = ts:newUdpTimestamper(txQueue, rxQueue, nil, true)
		rxQueue:enableTimestamps(tonumber(udp_dst_port))
	else
