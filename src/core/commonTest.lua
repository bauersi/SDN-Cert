...
function CommonTest.mapArgs(test, args, type, asTable, isFeature)
  ...
  logger.debug(testname .. ": map args for '" .. args .. "'")
  ...
  for i=1,#args do
    ...
    if (isVar) then
      ...
      if (not value) then
        logger.err("Could not map variable '" .. key .. "' for '" .. testname ..  "'")
        exit("Abort")
      end
    end
    line = line .. string.trim(tostring(value)) .. " "
    logger.debug(testname .. ": mapped '" .. arg .. "' to '" .. value .. "'")
  end
  line = string.trim(line)

  logger.debug(testname .. ": args mapped to '" .. line .. "'")
...
function CommonTest.checkLinkCount(test, arg, isFeature)
  ...
  if (linkId and linkId > #settings.ports) then
    local msg = (isFeature and "Disabled feature") or "Disabled test"
    local testname = (isFeature and test:getName()) or (test:getCompleteName())
    logger.warn(msg .. " '" .. testname .. "', link number out of range: " .. tostring(linkId) .. " of " .. tostring(#settings.ports))
    test:disable()
    return -1
  end
  return linkId
...
function CommonTest.readInFiles(test, folder, files, isFeature)
  ...
  for n,file in pairs(filelist) do
    ...
    if (not localfileExists(folder .. "/" .. file)) then
      local msg = (isFeature and "Disabled feature") or "Disabled test"
      local testname = (isFeature and test:getName()) or (test:getCompleteName())
      logger.warn(msg .. " '" .. testname .. "', missing file '" .. file .. "'")
      test:disable()
    else
