...
function Setup.checkOpenFlow()
  ...
  if (settings.config.simulate) then return false end

  local ports = OpenFlowDevice.create():getPorts()
  if ports == nil then
    return false
  else
...
function Setup.checkMoongen()
  ...
  if (true) then return true end   -- TODO: Find a way to check new moongen version
